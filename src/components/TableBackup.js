...
class Table extends Component {
  ...
  componentDidMount = async () => {
    ...
    //   let region = e.delegateTarget.tHead.rows[0].cells[this.cellIndex];
    //   let account = this.parentNode.cells[0];
  ...
  handleClick(accountNumber,Region){
    ...
    return this.state.accounts.map((account, index) => {
      ...
      return (
        <tr key={index}>
          ...
          <td onClick={() => this.handleClick(account_number,'us-east-1')}>{regions["us-east-2"]}</td>
